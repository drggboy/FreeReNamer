...
- 编译打包（生成可执行文件）
  ...
  默认按照 `src-tauri/tauri.conf.json` 中的 `bundle.targets` 打包（当前为 `all`），会同时生成 NSIS/MSI 安装包，并产出 `src-tauri/target/release/FreeReNamer.exe`。
...
- 仅生成 MSI 安装包
  ...
  只生成 MSI 安装包，省略 NSIS 安装包，但仍会生成 `src-tauri/target/release/FreeReNamer.exe`。
